...
//External pinion/ring gear ratio: ring=96 teeth, pinion=48 teeth -> GR = 18
...
// Carousel slot-1 home position: AS5600 raw count when slot 1 is aligned under the auger. CALIBRATION REQUIRED: rotate the carousel until slot 1 is perfectly aligned, read encoder.rawAngle() over Serial, and replace 512 with the observed value.
...
//The half-spur gear has teeth on exactly 180° of its circumference. One full M2 revolution therefore produces exactly ONE auger revolution (180° toothed arc drives the container full-spur gear through one complete turn, then the toothless 180° provides a mechanical cutoff). We define an "auger cycle" = 1 full M2 revolution = 1 auger revolution.Flow rate calibration is performed per-spice in grams per auger cycle.
...
// Back-purge: number of full M2 revolutions in reverse after a dispense. The toothed half sweeps back through the container gear, pulling spice back up the auger helix.  One full back-revolution clears the tube effectively.
...
// Per-spice dispense timeout (ms).  If target not reached within this time the firmware reports a fault and aborts the blend.
...
// Number of calibration data points to collect per slot stored in EEPROM; slope + intercept for each)
static constexpr uint8_t CALIB_POINTS_MIN = 3; //minimum before regression is valid

// Prediction horizon: we predict how many more cycles are needed and stop early to account for in-flight spice.  This "coast" allowance is computed dynamically from the linear model, but is clamped to [0, MAX_COAST_GRAMS].
