...
public:
    ...
    bool begin(uint32_t timeoutMs = 5000)
    {
        ...
        // Always tare on boot -- the scale must be empty at startup. If a bowl is already on it, readings will be wrong until the INDEXING tare runs. That's acceptable -- bowl detection uses MIN_BOWL_WEIGHT_G which is 20g, well above noise.
    ...
    void tare()
    {
        ...
        // Flush: read 5 real samples to confirm near-zero (don't update EMA -- just discard to let HX711 settle)
    ...
    // Blocking stable read -- waits for SETTLE_READS consecutive readings within STABLE_BAND_G. Max wait 2s.
    float readStable()
    {
        float prev = read();
        uint8_t count = 1;
