...
function SlotInput({
  ...
  const handleUnitSelect = (u: Unit) => {
    ...
    if (grams > 0) {
      ...
      converted = Math.min(
        Math.ceil(converted / newStep) * newStep,
        UNIT_MAX[u],
      );
    ...
    >
      ...
      <div className="flex items-center gap-2.5">
        ...
        >
          <FontAwesomeIcon
            icon={faMinus}
            style={{ fontSize: 12, color: "#C4BFB4" }}
          />
        </PressButton>
...
export function CustomRecipeScreen({ onSaved, onBack: _onBack }: Props) {
  ...
  const [units, setUnits] = useState<Record<number, Unit>>(
    Object.fromEntries(
      Object.keys(SPICE_LABELS).map((k) => [Number(k), "tsp" as Unit]),
    ),
  );
  ...
  const totalGrams = Object.entries(values).reduce(
    (sum, [slot, val]) => sum + toGrams(val, units[Number(slot)], Number(slot)),
    0,
  );
  ...
  const activeSpices = Object.entries(values)
    ...
    .map(([slot, v]) => ({
      slot: Number(slot),
      grams: toGrams(v, units[Number(slot)], Number(slot)),
    }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      gsap.to(saveRef.current, {
        ...
        ease: "power2.inOut",
        onComplete: () => {
          setTimeout(() => onSaved(result.recipe), 600);
        },
      });
    } catch (e) {
      ...
      <div className="flex flex-col px-5 pb-safe">
        ...
        <div data-s className="pt-3 pb-5">
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(90deg, rgba(200,105,42,0.5), transparent)",
            }}
          />
        </div>
        ...
        {error && (
          ...
          >
            <FontAwesomeIcon
              ...
              style={{ color: "#E07070", fontSize: 13 }}
            />
            <span
              className="font-body"
              style={{ fontSize: 14, color: "#E07070" }}
            >
              {error}
            </span>
          </div>
        ...
        <div data-s className="pb-2">
          ...
          >
            <FontAwesomeIcon
              icon={saved ? faCircleCheck : faFloppyDisk}
              style={{ fontSize: 15 }}
            />
