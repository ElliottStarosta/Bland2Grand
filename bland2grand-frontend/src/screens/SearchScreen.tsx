// Home screen with search bar, featured recipe cards, cuisine category shortcuts,
// and a custom blend entry point. Includes AI loading state for fuzzy searches.
...
// Quick-pick cuisine tiles mapped to backend search queries
const CATEGORIES = [
...
// Featured blend cards shown on the home screen
const FEATURED = [
...
// Rotating messages shown during AI search/generation
const AI_MESSAGES = [
...
// Loading overlay shown when no exact match is found and AI generation is triggered
function AiLoadingOverlay({ query }: { query: string }) {
  ...
  useEffect(() => {
    ...
    dots.forEach((dot, i) => {
      tl.to(
        dot,
        { scale: 1.7, opacity: 1, duration: 0.25, ease: "back.out(2)", yoyo: true, repeat: 1 },
        i * 0.11
      );
    });
  ...
  return (
    <div
      ref={overlayRef}
      className="mt-5 glass-card p-6 flex flex-col items-center gap-4"
    >
      {/* Spice color dots */}
      <div className="flex items-center gap-2">
        {Object.keys(SPICE_COLORS).map((slot) => (
          <span
            key={slot}
            ref={(el) => {
              dotsRef.current[Number(slot) - 1] = el;
            }}
      ...
      {/* Magic wand icon with spinning ring */}
      <div className="relative">
        <div
          className="w-14 h-14 rounded-full flex items-center justify-center"
          style={{
            background: "rgba(212,116,46,0.12)",
            border: "1px solid rgba(212,116,46,0.25)",
          }}
        >
          <FontAwesomeIcon
      ...
      <p ref={msgRef} className="text-sm text-muted font-body font-light">
        {AI_MESSAGES[0]}
      </p>
      ...
      <div className="w-full h-px rounded-full overflow-hidden bg-surface">
        <div
          ...
          style={{
            width: "100%",
            background:
              "linear-gradient(90deg, transparent, #D4742E, transparent)",
...
function FeaturedCard({
  ...
  useEffect(() => {
    gsap.fromTo(
      ref.current,
      { y: 18, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.4, delay, ease: "power3.out" }
    );
  }, [delay]);
  ...
  return (
    ...
    >
      {/* Spice color dots preview */}
      <div className="flex gap-1 flex-wrap">
        {slots.map((s) => (
          <span
            key={s}
            className="w-1.5 h-1.5 rounded-full"
            style={{ backgroundColor: SPICE_COLORS[s] }}
          />
        ))}
      </div>
      <div>
        <p className="text-[10px] text-muted font-body uppercase tracking-wider font-semibold mb-0.5">
          {category}
        </p>
        <p className="font-display text-sm font-semibold text-txt leading-tight">
          {name}
        </p>
      </div>
      <div className="flex items-center gap-1 mt-auto">
        <span className="text-[10px] text-accent font-body font-medium uppercase tracking-wider">
          Dispense
        </span>
        <FontAwesomeIcon
          icon={faChevronRight}
          className="text-accent text-[9px]"
        />
      </div>
    </button>
  );
...
function CategoryCard({
  ...
  return (
    ...
    >
      ...
      <span className="font-body font-medium text-txt text-[11px] leading-tight w-full break-words">
        {label}
      </span>
    </button>
  );
...
export function SearchScreen({ onResults, onSelect, onCustom }: Props) {
  ...
  // Entrance animation for search bar and body
  useEffect(() => {
  ...
  // Perform search with AI loading fallback
  const doSearch = useCallback(
    async (q: string) => {
      ...
      // Show AI loading if search takes too long
  ...
  // Load featured recipe by name
  const doFeatured = useCallback(
    async (name: string) => {
      setLoading(true);
      setError("");
      try {
  ...
  // Custom blend button with spring animation
    ...
    >
      <div className="flex flex-col px-5 pb-safe min-w-0 max-w-full">
        ...
        <div ref={searchRef} className="pt-2 w-full min-w-0 max-w-full">
          <div
            className="flex items-center gap-3 glass-card px-3 py-3.5 w-full min-w-0 max-w-full
              ...
              <div className="flex items-center gap-0.5 flex-shrink-0">
                {[0, 1, 2].map((i) => (
                  <span
                    ...
                    style={{
                      animation: `aiBounce 1s ease-in-out ${i * 0.15}s infinite`,
                    }}
                  />
                ))}
              </div>
        ...
        {/* Main content - hidden during AI loading */}
        {!aiLoading && (
          <div ref={bodyRef} className="flex flex-col gap-5 mt-5 w-full min-w-0 max-w-full">
            ...
            <section className="w-full min-w-0 max-w-full">
              ...
              <div
                className="flex gap-2.5 overflow-x-auto overflow-y-hidden pb-1 pr-5 w-full min-w-0 max-w-full"
            ...
            {/* Create custom blend entry point */}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="font-body font-medium text-txt text-sm">
                      Create Custom Blend
                    </p>
                    <p
