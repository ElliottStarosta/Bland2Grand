...
const PHASE_DIMS = {
  ...
  2: "rgba(6,182,212,.18)",
  3: "rgba(16,185,129,.18)",
  4: "rgba(245,158,11,.18)",
  5: "rgba(168,85,247,.18)",
};
...
function initSidebarResize() {
  ...
  window.addEventListener("resize", function () {
    applySidebarWidth(readCurrentW());
  });
}
...
async function saveToFile() {
  try {
    ...
    showSaveIndicator(
      res.ok ? "Saved to disk \u2713" : "Save failed \u2717",
      !res.ok,
    );
  } catch {
...
function showSaveIndicator(msg, isError) {
  ...
  el.style.cssText =
    ...
    "color:" +
    (isError ? "#f43f5e" : "#22c55e") +
    ";margin-left:12px;transition:opacity .5s";
  el.textContent = msg;
  bar.appendChild(el);
  setTimeout(function () {
    el.style.opacity = "0";
...
setInterval(function () {
  if (tasks && tasks.length) saveToFile();
}, 30000);
...
const dateToX = function (d) {
  return daysBetween(PROJECT_START, parseTaskDate(d)) * PX_PER_DAY;
};
const totalDays = function () {
  return Math.ceil(daysBetween(PROJECT_START, PROJECT_END)) + 2;
};
const f = function (n) {
  return (+n).toFixed(1);
};
...
function getColor(task) {
  return task.type === "milestone"
    ? "#f43f5e"
    : PHASE_COLORS[task.phase] || "#8b949e";
}
function isVisible(task) {
  ...
  if (searchQ) {
    ...
    return (
      task.name.toLowerCase().includes(q) || task.id.toLowerCase().includes(q)
    );
  }
  return true;
}
...
function renderSidebar() {
  ...
  tasks.forEach(function (task) {
    ...
    const hasKids = tasks.some(function (t) {
      return getParentId(t.id) === task.id && isVisible(t);
    });
    const deps = task.dep || "\u2014";
    ...
    const tog = hasKids
      ? '<button class="toggle-btn' +
        (collapsed[task.id] ? " coll" : "") +
        '" onclick="event.stopPropagation();toggleCollapse(\'' +
        task.id +
    ...
    if (task.type === "phase") {
      frags.push(
        '<div class="' +
          ...
          '">' +
          '<div class="row-inner">' +
          '<div class="row-name"><div class="phase-pill" style="background:' +
          color +
          '"></div>' +
          ...
          '<div class="row-dur">' +
          task.dur +
          'd</div><div class="row-dep">\u2014</div><div></div><div></div>' +
          "</div></div>",
      );
      return;
    }
    ...
    const pct =
      task.type === "milestone" ? (isDone ? 100 : 0) : task.progress || 0;
    const pcol = getProgressColor(pct);
    const msIc =
      task.type === "milestone"
        ? '<i class="fa-solid fa-diamond" style="color:#f43f5e;font-size:10px;flex-shrink:0"></i>'
        : "";
    const pill =
      task.type !== "milestone"
        ? '<div class="phase-pill" style="background:' +
          color +
          ';opacity:.5"></div>'
        : "";
    const tag =
      task.type !== "phase" && task.type !== "milestone" ? task.id : "";
    ...
    frags.push(
      '<div class="' +
        ...
        "</span></div>" +
        '<div class="row-dur">' +
        (task.dur > 0 ? task.dur + "d" : "\u2014") +
        "</div>" +
        '<div class="row-dep" title="' +
        (task.dep || "") +
        '">' +
        deps +
        "</div>" +
        '<div class="row-prog" onclick="event.stopPropagation();quickProgress(\'' +
        task.id +
        '\')" title="Click to cycle progress">' +
        '<div class="prog-bar-wrap"><div class="prog-bar-fill" style="width:' +
        pct +
        "%;background:" +
        pcol +
        '"></div></div>' +
        ...
        '" onclick="event.stopPropagation();toggleDone(\'' +
        task.id +
        "')\">" +
        '<i class="fa-solid ' +
        (isDone ? "fa-check-circle" : "fa-circle") +
        '"></i></button>' +
        '<button class="act-btn" title="Edit" onclick="event.stopPropagation();openEdit(\'' +
        task.id +
        "')\">" +
...
function renderGantt() {
  ...
  months.forEach(function (m) {
    ...
    hdrHtml +=
      '<div class="gh-month" style="width:' +
      mw +
      'px">' +
  ...
  for (let d = 0; d < total; d++) {
    ...
    if (dow === 0 || dow === 6)
      html +=
        '<div class="g-weekend" style="left:' +
        d * PX_PER_DAY +
        "px;width:" +
        PX_PER_DAY +
        'px"></div>';
  ...
  months.forEach(function (m) {
    html +=
      '<div class="g-line month" style="left:' +
      m.startDay * PX_PER_DAY +
      'px"></div>';
    for (let i = 7; i < m.days.length; i += 7)
      html +=
        '<div class="g-line" style="left:' +
        (m.startDay + i) * PX_PER_DAY +
        'px"></div>';
  });
  ...
  tasks.forEach(function (task) {
    ...
    if (task.type === "phase") {
      bar =
        '<div class="gbar gbar-phase" style="left:' +
        x +
        "px;width:" +
        task.dur * PX_PER_DAY +
        "px;background:" +
      ...
      bar =
        '<div class="gbar gbar-ms" style="left:' +
        (x - 8) +
        'px;top:8px;background:#f43f5e"' +
        " onmouseenter=\"showTT(event,'" +
        task.id +
        '\')" onmouseleave="hideTT()" onclick="openEdit(\'' +
        task.id +
        "')\"></div>";
    } else if (task.dur > 0) {
      ...
      bar =
        ...
        color +
        ";opacity:" +
        (task.type === "task" ? 1 : 0.8) +
        (isDone ? ";filter:brightness(.7)" : "") +
        '"' +
        " onmouseenter=\"showTT(event,'" +
        task.id +
        '\')" onmouseleave="hideTT()" onclick="openEdit(\'' +
        task.id +
        "')\">" +
        '<div class="gbar-prog" style="width:' +
        pct +
        '%"></div>' +
        '<span class="gbar-label">' +
        lbl +
        "</span>" +
        (isDone
          ? '<i class="fa-solid fa-check" style="margin-left:auto;font-size:8px;position:relative;z-index:2"></i>'
          : "") +
        "</div>";
    }
    ...
    html +=
      '<div class="' + gc + '" data-id="' + task.id + '">' + bar + "</div>";
  });
  ...
  pane.onscroll = function () {
    sidebar.parentElement.scrollTop = pane.scrollTop;
  };
  sidebar.parentElement.onscroll = function () {
    pane.scrollTop = sidebar.parentElement.scrollTop;
  };
}
...
function drawArrows() {
  ...
  for (var ei = 0; ei < els.length; ei++) {
    ...
    rowInfo[el.getAttribute("data-id")] = {
      topY: t,
      midY: t + h / 2,
      botY: t + h,
    };
  ...
  function geom(task) {
    ...
    if (!g) {
      ...
      if (task.type === "milestone") {
        ...
        g = {
          L: x,
          R: x + Math.max(task.dur * PX_PER_DAY, 1),
          midY: ri.midY,
          topY: ri.topY,
          botY: ri.botY,
        };
  ...
  function filterExForMode(xs, sx, ax, mode) {
    ...
    if (mode === "right") {
      ...
      for (i = 0; i < xs.length; i++) {
        if (xs[i] > need) out.push(xs[i]);
      }
      if (!out.length) {
        for (var vr = 0; vr < 40; vr++) out.push(need + 6 + vr * 8);
      }
    } else if (mode === "left") {
      var cap = Math.min(sx, ax) - pad;
      for (i = 0; i < xs.length; i++) {
        if (xs[i] < cap) out.push(xs[i]);
      }
      if (!out.length) {
        for (var vl = 0; vl < 40; vl++) out.push(cap - 6 - vl * 8);
      }
    }
  ...
  function routeEdge(sx, sy, dg, chain, skipIdSrc, tieBump) {
    ...
    var aLx = dg.L - MARG;
    var aLy = dg.midY;
    var aTx = dg.L + (dg.R - dg.L) * frac;
    var aTy = dg.topY - MARG;
    var aBx = dg.L + (dg.R - dg.L) * frac;
    var aBy = dg.botY + MARG;
    var candidates = [];
  ...
  function extractVerticals(pts) {
    ...
    for (var i = 0; i < pts.length - 1; i++) {
      if (Math.abs(pts[i].x - pts[i + 1].x) < 0.5) {
        out.push({
          x: pts[i].x,
          y0: Math.min(pts[i].y, pts[i + 1].y),
          y1: Math.max(pts[i].y, pts[i + 1].y),
        });
      }
    }
    return out;
  ...
  function extractHorizontals(pts) {
    ...
    for (var i = 0; i < pts.length - 1; i++) {
      if (Math.abs(pts[i].y - pts[i + 1].y) < 0.5) {
        out.push({
          y: pts[i].y,
          x0: Math.min(pts[i].x, pts[i + 1].x),
          x1: Math.max(pts[i].x, pts[i + 1].x),
        });
      }
    }
    return out;
  ...
  function pathConflictsPlaced(pts, vPlaced, hPlaced) {
    ...
    for (var a = 0; a < vi.length; a++) {
      if (vSegConflictsPlaced(vi[a], vPlaced)) return true;
    }
    for (var b = 0; b < hi.length; b++) {
      if (hSegConflictsPlaced(hi[b], hPlaced)) return true;
    }
    return false;
  }
  ...
  for (var ei = 0; ei < edgeList.length; ei++) {
    ...
    for (lane = 0; lane < 55; lane++) {
      ...
      if (!pathConflictsPlaced(tryPts, placedV, placedH)) {
        pts0 = tryPts;
        break;
      }
    }
    ...
    for (var j = i + 1; j < arrows.length; j++) {
      ...
      for (var asi = 0; asi < A.length - 1; asi++) {
        for (var bsi = 0; bsi < B.length - 1; bsi++) {
          ...
          var hMinX = Math.min(hP.x, hQ.x),
            hMaxX = Math.max(hP.x, hQ.x);
          var vMinY = Math.min(vP.y, vQ.y),
            vMaxY = Math.max(vP.y, vQ.y);
          var vx = vP.x,
            hy = hP.y;
          if (
            vx > hMinX + 1 &&
  ...
  function arrowHeadPolygonPoints(tipX, tipY, prevX, prevY) {
    ...
    if (Math.abs(dx) < SNAP_EPS && Math.abs(dy) >= SNAP_EPS) {
      var ux = 0;
      var uy = dy >= 0 ? 1 : -1;
      var bx = tipX - ux * ARROW_TIP_INSET,
        by = tipY - uy * ARROW_TIP_INSET;
      var px = -uy * ARROW_HALF_W,
        py = ux * ARROW_HALF_W;
      return (
        tipX +
        "," +
  ...
  function toD(pts, segCross) {
    ...
    for (var i = 1; i < pts.length; i++) {
      ...
      var tList = (segCross[i - 1] || []).slice().sort(function (a, b) {
        return a - b;
      });
      if (tList.length > 0) {
        ...
        var p1x = curr.x - (inDx / inL) * r,
          p1y = curr.y - (inDy / inL) * r;
        var p2x = curr.x + (outDx / outL) * r,
          p2y = curr.y + (outDy / outL) * r;
        d +=
          " L " +
          f(p1x) +
          " " +
  ...
  for (var ai2 = 0; ai2 < arrows.length; ai2++) {
    ...
    poly.setAttribute(
      "points",
      arrowHeadPolygonPoints(tip.x, tip.y, prv.x, prv.y),
    );
...
function renderStats() {
  var nonPhase = tasks.filter(function (t) {
    return t.type !== "phase";
  });
  var totalT = nonPhase.length;
  var milestones = tasks.filter(function (t) {
    return t.type === "milestone";
  }).length;
  var done = nonPhase.filter(function (t) {
    return t.progress >= 100;
  }).length;
  var totalDur = tasks
    .filter(function (t) {
      return t.type === "subtask";
    })
    .reduce(function (s, t) {
      return s + (t.dur || 0);
    }, 0);
  ...
  document.getElementById("statusBar").innerHTML =
    '<div class="stat"><i class="fa-solid fa-list-check"></i> <strong>' +
    totalT +
    "</strong> tasks</div>" +
    '<div class="stat"><i class="fa-solid fa-diamond" style="color:#f43f5e"></i> <strong>' +
    milestones +
    "</strong> milestones</div>" +
    '<div class="stat"><i class="fa-solid fa-clock"></i> <strong>' +
    totalDur.toFixed(1) +
    "</strong> working days</div>" +
    '<div class="stat"><i class="fa-solid fa-circle-check" style="color:#22c55e"></i> <strong>' +
    done +
    "</strong> / " +
    totalT +
    " complete (<strong>" +
...
function showTT(e, id) {
  ...
  document.getElementById("tooltip").innerHTML =
    '<div class="tt-title" style="color:' +
    color +
    '">' +
    task.id +
    " \u00b7 " +
    ...
    task.name +
    "</div>" +
    '<div class="tt-row"><i class="fa-solid fa-calendar-day"></i> Start: ' +
    task.start +
    "</div>" +
    '<div class="tt-row"><i class="fa-solid fa-hourglass-half"></i> Duration: ' +
    (task.dur > 0 ? task.dur + " days" : "\u2014") +
    "</div>" +
    (task.dep
      ? '<div class="tt-row"><i class="fa-solid fa-arrow-right-from-bracket"></i> Deps: ' +
        task.dep +
        "</div>"
      : "") +
    '<div class="tt-row"><i class="fa-solid fa-chart-simple"></i> Progress: ' +
    (task.progress || 0) +
    "%</div>" +
    (task.notes
      ? '<div class="tt-notes"><i class="fa-solid fa-note-sticky"></i> ' +
        task.notes +
        "</div>"
      : "");
...
//ZOOM / FILTER
function setZoom(v) {
  PX_PER_DAY = parseInt(v);
  renderGantt();
}
function filterByPhase(v) {
  filterPhase = v;
  render();
}
...
function openAdd() {
  ...
  ["fId", "fName", "fDep", "fNotes"].forEach(function (id) {
...
async function exportImage() {
  ...
  if (!window.html2canvas) {
    await new Promise((resolve, reject) => {
      ...
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = '<i class="fa-solid fa-image"></i> Export PNG';
      }
      throw err;
    });
  }
...
function resetData() {
  if (
    !confirm(
      "Reset ALL data to original tasks.json? All progress will be lost.",
    )
  )
    return;
...
document
  .getElementById("fProg")
  .addEventListener("input", syncProgSliderVisual);
