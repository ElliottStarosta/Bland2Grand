...
function lighten(hex: string, ratio: number): string {
  ...
  const r = Math.min(
    255,
    Math.floor(((n >> 16) & 0xff) + (255 - ((n >> 16) & 0xff)) * ratio),
  );
  const g = Math.min(
    255,
    Math.floor(((n >> 8) & 0xff) + (255 - ((n >> 8) & 0xff)) * ratio),
  );
...
function GrainLayer({ color, yTop, yBot, W, slot, isTop }: GrainProps) {
  ...
  if (isTop) {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      pts.push(
        `${i === 0 ? "M" : "L"} ${px.toFixed(1)} ${(yTop + noise).toFixed(1)}`,
      );
    }
...
export function Bowl({ slots, totalTarget, totalWeight, activeSlot }: Props) {
  ...
  useEffect(() => {
    ...
    tl.fromTo(
      dripRef.current,
      {
        attr: { y: mouthY - 28, height: 14, width: 6, x: cx - 3, rx: 3 },
        opacity: 1,
      },
      {
        attr: { y: mouthY + 4, height: 4, width: 4, x: cx - 2, rx: 2 },
        opacity: 0.1,
        duration: 0.4,
        ease: "power3.in",
      },
