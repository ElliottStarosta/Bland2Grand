...
// Carousel motor (M1 / NEMA 23 / TB6600) - rotates the spice carousel to
// bring the selected slot under the auger
...
// Empirical correction added per slot to counter accumulated mechanical
// backlash/slip that STEPS_PER_SLOT alone doesn't fully capture
static constexpr uint16_t STEPS_PER_SLOT_CORRECTION = 143;

// Settle delay after index before dispense begins (ms) - lets carousel
// vibration die down before the auger starts turning
...
// Delay after back-purge before disabling coils (ms) - lets the auger
// fully stop before power is cut, avoiding a coasting drift
...
// Three-stage closed-loop speed ramp keyed to dispense progress
// (steps_done / total_steps). Slows the auger down as it approaches target
// to reduce overshoot.
...
// Phase 1 (bulk/coast) stops at this fraction of target weight.
// In-flight spice + any clump release must not push the total past target.
static constexpr float COAST_UNDERSHOOT_RATIO = 0.85f;

// Phase 3 (nudge): how many auger cycles per nudge tap.
