...
\paragraphshot[0.6\linewidth]{The laptop's IP address is printed in the Vite terminal panel when it starts. On the Arduino's serial monitor you will also see the IP it received from the hotspot, which confirms the two devices are on the same subnet.}{images/r4_vite_server.png}{Vite dev server terminal output showing the network URL to open on the phone.}
...
Once the web app is open on a phone, use it the same way regardless of which board is running underneath: search for a dish (or build a custom blend), pick a result, set the number of servings, tap \textbf{Dispense}, and watch the live progress screen until the Complete screen appears.
